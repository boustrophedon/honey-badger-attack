...
class AttackSystem(System):
	def __init__(self, world):
		...
		self.world.subscribe_event("MobFireLaser", self.onMobFireLaser)
		self.world.subscribe_event("PlayerAttack", self.onPlayerAttack) 
	...
	def onMobFireLaser(self, event_type, event):
		if event.mob == None:
			self.fire_all_lasers()
		else:
			self.fire_laser(event.mob)

	def onPlayerAttack(self, event_type, event):
		pass
